  ...
  return (
    <main className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] transition-colors duration-300">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <header className="flex items-start justify-between rounded-3xl border border-[var(--border-primary)] bg-[var(--bg-surface)] p-6">
          <div>
            ...
            <p className="mt-2 text-sm text-[var(--text-secondary)]">Implementação própria multi-modo com suporte dark/light.</p>
          </div>
          <ThemeToggle />
        </header>

        <section className="mt-6 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <aside className="rounded-2xl border border-[var(--border-primary)] bg-[var(--bg-surface)] p-4">
              ...
              {modes.map((m) => (
                <button
                  ...
                  className={`rounded-xl border px-3 py-2 text-sm font-bold ${
                    mode === m.id ? "border-cyan-300 bg-cyan-300/15" : "border-[var(--border-primary)] bg-[var(--bg-muted)]"
                  }`}
                >
                  {m.label}
                </button>
            ...
            <textarea
              ...
              className="mt-4 h-40 w-full rounded-xl border border-[var(--border-primary)] bg-[var(--bg-muted)] p-3 text-sm"
            />

            <button
              onClick={run}
              disabled={loading}
              className="mt-3 w-full rounded-xl bg-[var(--accent)] px-4 py-2 text-sm font-black text-slate-950 disabled:opacity-50"
            >
              {loading ? "Executando..." : `Executar ${modes.find((m) => m.id === mode)?.label}`}
            </button>
          </aside>

          <article className="rounded-2xl border border-[var(--border-primary)] bg-[var(--bg-muted)] p-4">
            <h2 className="text-lg font-extrabold">Resposta da API</h2>
            <pre className="mt-3 h-[460px] overflow-auto rounded-xl border border-[var(--border-primary)] bg-[var(--bg-surface)] p-3 text-xs text-[var(--text-primary)]">
              {result}
            </pre>
          </article>
        </section>
