...
const AIGuideWidget = dynamic(() => import("@/app/components/ai-guide-widget"), {
  ssr: false,
  loading: () => (
    <section className="rounded-3xl border border-white/10 bg-white/[0.05] p-5 backdrop-blur-xl">
      <p className="text-sm text-slate-300">Carregando assistente...</p>
    </section>
  ),
});
...
function ItemImage({ src, alt }: { src: string; alt: string }) {
  ...
  if (isRemote) {
    return <img src={src} alt={alt} className="mb-3 h-36 w-full rounded-2xl object-cover" loading="lazy" width={1200} height={675} />;
  }
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {items.map((item) => (
            <article key={item.id} className="rounded-3xl border border-white/10 bg-white/[0.05] p-5 backdrop-blur-xl">
              <ItemImage src={item.imageUrl || imageByItemId[item.id] || "/illustrations/mk-p1.svg"} alt={item.title} />
